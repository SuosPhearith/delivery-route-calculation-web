...
import { message, Modal, Popconfirm, PopconfirmProps } from "antd";
...
const CaseComponent = () => {
  ...
  const handleEdit = (item: CreateCaseInterface) => {
    setValue("name", item.name);
    setValue("containerHeight", item.caseHeight);
    setValue("containerWeight", item.caseWeight);
    setValue("containerLenght", item.caseLenght);
    showModal();
    setUpdateId(item.id);
  };
  type Inputs = {
  ...
  const onSubmit: SubmitHandler<Inputs> = async (data) => {
    if (updateId) {
      const caseData: CreateCaseInterface = {
        id: updateId,
        name: data.name,
        caseLenght: data.containerLenght,
        caseWeight: data.containerWeight,
        caseHeight: data.containerHeight,
      };
  ...
  //fectch
  const { data, isLoading, isError } = useQuery<CaseResponse>({
    ...
    <section className="case mx-auto px-1">
      ...
      >
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          <input
            {...register("containerWeight", { required: true })}
            type="number"
