...
const DriverComponent = () => {
  ...
  const { data, isError, isLoading } = useQuery({
    ...
    queryFn: () => getAllDriver(page, limit, status, query),
  });
  useEffect(() => {
  ...
  return (
    <section className="container mx-auto px-1">
      ...
      <div className="mt-3 md:flex md:items-center md:justify-between">
        <div className="inline-flex divide-x overflow-hidden rounded-md border bg-white dark:divide-gray-700 dark:border-gray-700 dark:bg-gray-900 rtl:flex-row-reverse">
          <button
            onClick={() => setStatus("")}
            className={`${status === "" ? "text-primary " : "text-black "} rounded-sm px-5 py-2 text-xs transition-colors duration-200 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-gray-800 sm:text-sm`}
          >
            ALL
          </button>
          <button
            onClick={() => setStatus("ACTIVE")}
            className={`${status === "ACTIVE" ? "text-primary " : "text-black "}rounded-sm px-5 py-2 text-xs transition-colors duration-200 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-gray-800 sm:text-sm`}
          >
            ACTIVE
          </button>
          <button
            onClick={() => setStatus("PENDING")}
            className={`${status === "PENDING" ? "text-primary " : "text-black "}rounded-sm px-5 py-2 text-xs transition-colors duration-200 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-gray-800 sm:text-sm`}
          >
            PENDING
          </button>
          <button
            onClick={() => setStatus("INACTIVE")}
            className={`${status === "INACTIVE" ? "text-primary " : "text-black "}rounded-sm px-5 py-2 text-xs transition-colors duration-200 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-gray-800 sm:text-sm`}
          >
            INACTIVE
          </button>
        </div>
